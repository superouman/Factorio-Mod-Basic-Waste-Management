...
local itemName = {
	...
	--
	--inserter.lua
	"long-handed-inserter", "fast-inserter", "filter-inserter", "stack-inserter", "stack-filter-inserter", "inserter",
	--module.lua
...
local itemNameFluids = {"basic-oil-processing", "advanced-oil-processing", "coal-liquefaction", "heavy-oil-cracking", "light-oil-cracking",
	"sulfuric-acid", "plastic-bar", "solid-fuel-from-light-oil", "solid-fuel-from-heavy-oil", "solid-fuel-from-petroleum-gas", "sulfur", "lubricant"}
local recipeData
-- local wasteAmount
...
for i, recipeName in pairs(itemName)
	do
		recipeData = data.raw.recipe[recipeName]
		-- wasteAmount = 0
		...
		if recipeData.ingredients == nil then
			...
			-- for i, ingredient in pairs(recipeData.ingredients) do
			-- 	wasteAmount = wasteAmount + ingredient[2]
			-- end
			recipeData.results = {
